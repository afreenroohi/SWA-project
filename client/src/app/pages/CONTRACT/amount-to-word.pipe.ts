...
export class AmountToWordPipe implements PipeTransform {
    transform(value: any, args?: any): any {
        if (value) {
            ...
            let amount = value.toString().split(".");
            let price: any = amount[0];
            let pointer: any = amount.length > 0 ? amount[1] : null;
            ...
            var riyal = "",
                ...
                paisaWords = [],
                paisa = "";
            if (price += "", isNaN(parseFloat(price))) riyal = "";
                ...
                riyal = words.reverse().join("")
            } else riyal = "";
            if (riyal)
                riyal = `${riyal} Saudi Riyals`
            if (pointer != "00") {
                digitIndex = 0;
                digit = 0;
                nextDigit = 0;

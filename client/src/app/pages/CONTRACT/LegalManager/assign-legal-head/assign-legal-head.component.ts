...
@Component({
  selector: 'app-assign-legal-head',
  templateUrl: './assign-legal-head.component.html',
  styleUrls: ['./assign-legal-head.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class AssignLegalHeadComponent implements OnInit {
  ProxyUserId = 'TSUDHA';
    ...
    if (this.award_number) {
      ...
      this.api.post("getDetails", { ...AwardNum, userName: this.ProxyUserId }).subscribe(
        (res) => {
          ...
          this.isCommRegNum = this.contractDetails.RegType == 'C' ? true : false;
          if (this.cs.userLanguage == 'ar') {
            this.ContractForm.get('AmountInWords')?.setValue(this.contractDetails.AmountInAr);
          } else {
            this.ContractForm.get('AmountInWords')?.setValue(this.contractDetails.AmountInEn);
      ...
      this.router.navigateByUrl('contract/legalManagerDashboard/Assign')
      if (this.cs.userLanguage == 'en') {
  ...
  handleOk(): void {
    ...
    this.assignPayload.LglUnitHead = this.selectedOfficer;
    ...
    this.api.post('assignOfficer', dataPay).subscribe((res) => {
      if (res == 204) {
        ...
        if (this.cs.userLanguage == 'en') {
          this.message.create('success', 'Legal Unit Head assigned successfully')
        } else {
          this.message.create('success', 'تم تعيين رئيس الوحدة القانونية بنجاح')
        }
        this.router.navigateByUrl('contract/legalManagerDashboard/Assign')
      }
    });

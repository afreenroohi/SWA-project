...
//  approval of contract detail component
@Component({
  selector: 'app-approve-contract-ch',
  templateUrl: './approve-contract-ch.component.html',
  styleUrls: ['./approve-contract-ch.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class ApproveContractCHComponent implements OnInit {
  ProxyUserId = 'TSUDHA';
  constructor(
  ...
  getOTPModelAssign: boolean = false;
  user_name: any;
  ...
  ngOnInit(): void {
    ...
    this.unit_officer = history.state.unit_officer;
    ...
    if (this.award_number) {
      ...
      this.api.post("getDetails", { ...AwardNum, userName: this.ProxyUserId }).subscribe(
        (res) => {
          ...
          this.checkRFP = this.contractDetails.PHtoRFP == 'X' ? false : true;
          this.spinner.hide();
        },
        (err) => {
          console.log(err);
  ...
  approve() {
    ...
    //this.approvePayload.PHtoRFP = this.checkRFP ? 'Y' : 'N';
    ...
    this.api.post('assignOfficer', dataPay).subscribe((res) => {
      if (res == 204) {
        ...
        this.router.navigateByUrl('contract/legalHeadDashboard/approve')
      }
    });
  }
